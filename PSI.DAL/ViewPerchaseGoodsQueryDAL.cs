...
namespace PSI.DAL
{
    public class ViewPerchaseGoodsQueryDAL
    {
        /// <summary>
        /// 采购统计  分页
        /// </summary>
        ...
        /// <returns></returns>
        public DataTable GetPerTotal(int id, int typeId, string sqlTotal, string strWhere, SqlParameter[] paras)
        {
            string strName = "";
            switch (typeId)
            {
            ...
            string sql = $"select {sqlTotal} from ViewPerGoodsQuery where {strName}=@id and IsChecked=1" + strWhere;
