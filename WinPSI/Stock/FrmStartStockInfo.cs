...
namespace WinPSI.Stock
{
        public partial class FrmStartStockInfo : SheetFormParent
        {
                ...
                private void InitInfo()
                {
                        ...
                        {
                                //获取指定入库单信息   
                                StockStoreInfoModel stockInfo = stockBLL.GetStockInfo(stockId);
                                if (stockInfo != null)
                                {
                                        ...
                                        store = new StoreInfoModel()
                                        {
                                                ...
                                                StoreName = storeBLL.GetStoreInfo(stockInfo.StoreId).StoreName
                                        };
                                        ...
                                        //获取该单据相关的商品明细列表
                                        List<ViewStStockGoodsInfoModel> stockGoods = stockBLL.GetStockGoodsList(stockId);
                ...
                private void tsbtnSave_Click(object sender, EventArgs e)
                {
                        Action act = () =>
                        {
                                ...
                                if (stockId == 0)//新单保存
                                {
                                        ...
                                        string reStr = stockBLL.AddStockInfo(stockInfo, listGoods);
                                ...
                                else//修改保存   新增后修改     单据列表页进来 
                                {
                                        ...
                                        bool bl = stockBLL.UpdateStockInfo(stockInfo, listGoods);
                                        if (bl)
                                        {
                ...
                private void tsbtnCheck_Click(object sender, EventArgs e)
                {
                        Action act = () =>
                        {
                                ...
                                {
                                        bool bl = stockBLL.CheckStockInfo(stockId, uName, store.StoreId);
                                        if (bl)
                                        {
                                                lblCheckState.Text = "已审核";
                ...
                private void tsmiRedChecked_Click(object sender, EventArgs e)
                {
                        Action act = () =>
                        {
                                ...
                                {
                                        bool bl = stockBLL.RedCheckStockInfo(stockId, store.StoreId);
                                        if (bl)
                                        {
                ...
                private void tsmiNoUse_Click(object sender, EventArgs e)
                {
                        Action act = () =>
                        {
                                ...
                                {
                                        bool bl = stockBLL.NoUseStockInfo(stockId);
                                        if (bl)
                                        {
