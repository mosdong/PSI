...
namespace WinPSI.BM
{
        public partial class FrmStoreTypeList : Form
        {
                ...
                private void LoadStoreTypeList()
                {
                        ...
                        List<StoreTypeInfoModel> typeList = stBLL.LoadAllStoreTypes(isShowDel);
                ...
                private void DeleteStoreType(int isDeleted,StoreTypeInfoModel st)
                {
                        ...
                        if (MsgBoxHelper.MsgBoxConfirm(msgTitle, $"您确定要{delTypeName}该仓库类别？") == DialogResult.Yes)
                        {
                                ...
                                switch (isDeleted)
                                {
                                        case 1://删除
                                                //如果该类别添加了仓库，不允许删除
                                                bool hasAddStores = stBLL.IsAddStores(st.STypeId);
                                                if (!hasAddStores )
                                                {
                                                        bl = stBLL.LogicDelete(st.STypeId);
                                                }
                                                else
                                                {
                                                ...
                                                break;
                                        case 0://恢复
                                                bl = stBLL.Recover(st.STypeId);
                                                break;
                                        case 2://移除
                                                bl = stBLL.Delete(st.STypeId);
                                                break;
                                }
                        ...
                        Action act = () =>
                        {
                                ...
                                if (MsgBoxHelper.MsgBoxConfirm(title, "您确定要删除选择的这些仓库类别信息吗？") == DialogResult.Yes)
                                {
                                        ...
                                        foreach (DataGridViewRow row in dgvStoreTypeList.SelectedRows)
                                        {
                                                ...
                                                //如果该类别添加了仓库，不允许删除
                                                bool hasAddStores = stBLL.IsAddStores(stInfo.STypeId);
                                                if (!hasAddStores)
                                                {
                ...
                private void btnSave_Click(object sender, EventArgs e)
                {
                        ...
                        {
                                if (stBLL.ExistsStoreType(typeName))
                                {
