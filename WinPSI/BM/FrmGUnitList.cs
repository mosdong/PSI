...
namespace WinPSI.BM
{
        public partial class FrmGUnitList : Form
        {
                ...
                private void LoadGUnitList()
                {
                        ...
                        List<GoodsUnitInfoModel> list = guBLL.GetAllUnits(isDeleted);
                ...
                private void DeleteUnit(GoodsUnitInfoModel guInfo, int actType)
                {
                        ...
                        if (MsgBoxHelper.MsgBoxConfirm(title, $"您确定要{actMsg}该计量单位吗？") == DialogResult.Yes)
                        {
                                ...
                                {
                                        case 0:
                                                bl = guBLL.Recover(guInfo.GUnitId);
                                                break;
                                        case 1:
                                                if (!guBLL.GetGoodsUnitUse(guInfo.GUnitName))
                                                        bl = guBLL.LogicDelete(guInfo.GUnitId);
                                                else
                                                {
                                                ...
                                                break;
                                        case 2:
                                                bl = guBLL.Delete(guInfo.GUnitId);
                                                break;
                                }
                ...
                private void tsbtnDelete_Click(object sender, EventArgs e)
                {
                        Action act = () =>
                        {
                                ...
                                if (MsgBoxHelper.MsgBoxConfirm(title, "您确定要删除选择的这些计量单位信息吗？") == DialogResult.Yes)
                                {
                                        ...
                                        bool bl = guBLL.LogicDeleteList(guIds);
                                        string sucMsg = bl ? "成功" : "失败";
