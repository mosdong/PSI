                ...
                private void DeleteGoodsInfo(int isDeleted, ViewGoodsInfoModel goodsInfo)
                {
                        ...
                        if (MsgBoxHelper.MsgBoxConfirm(msgTitle, $"您确定要{delTypeName}该商品信息？") == DialogResult.Yes)
                        {
                                ...
                                switch (isDeleted)
                                {
                                        case 1://删除
                                                //如果商品在使用中，不允许删除
                                                bool IsGoodsUse = goodsBLL.CheckIsGoodsUse(goodsInfo.GoodsId);
                                                if (!IsGoodsUse)
                                                {
                                                        bl = goodsBLL.DeleteGoodsInfo(goodsInfo.GoodsId);
                                                }
                                                else
                                                {
                                                ...
                                                break;
                                        case 0://恢复
                                                bl = goodsBLL.RecoverGoodsInfo(goodsInfo.GoodsId, uName);
                                                break;
                                        case 2://移除
                                                bl = goodsBLL.RemoveGoodsInfo(goodsInfo.GoodsId);
                                                break;
                                }
