...
namespace WinPSI.Sale
{
        public partial class FrmSaleOutStore : SheetFormParent
        {
                ...
                private void InitInfo()
                {
                        ...
                        {
                                //加载指定的销售单信息
                                SaleOutStoreInfoModel saleInfo = saleBLL.GetSaleInfo(saleId);
                                if (saleInfo != null)
                                {
                                        ...
                                        //加载商品列表
                                        List<ViewSaleGoodsInfoModel> saleGoodsList = saleBLL.GetSaleGoodsList(saleId);
                        ...
                        isOpened = sysBLL.GetOpenState(1);//获取开账状态
                        if (!isOpened)
                        {
                ...
                private void tsbtnSave_Click(object sender, EventArgs e)
                {
                        Action act = () =>
                        {
                                ...
                                {
                                        //reStr  saleId,SaleNo
                                        string reStr = saleBLL.AddSaleInfo(saleInfo, listGoods);
                                ...
                                else//修改保存   新增后修改     单据列表页进来 
                                {
                                        ...
                                        bool bl = saleBLL.UpdateSaleInfo(saleInfo, listGoods);
                                        if (bl)
                                        {
                ...
                private void tsbtnCheck_Click(object sender, EventArgs e)
                {
                        Action act = () =>
                        {
                                ...
                                if (MsgBoxHelper.MsgBoxConfirm("销售单审核", "你确定要审核该销售单吗？") == DialogResult.Yes)
                                {
                                        List<ViewSaleGoodsInfoModel> listGoods = dgvGoods.DataSource as List<ViewSaleGoodsInfoModel>;
                                        //检查库存
                                        string reStr = stockBLL.CheckGoodsCurCount(listGoods, store.StoreId);
                                        if(!string.IsNullOrEmpty(reStr))
                                        {
